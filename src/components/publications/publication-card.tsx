...
export default function PublicationCard({ paper }: PaperProps) {
  ...
  const highlightKeywords = (text: string) =>
    text
      ...
      .replace(
        /(Conference\s*Rank\s*:\s*(Core\s+)?[ABC])/gi,
        `<strong class="text-green-600">$1</strong>`
      )
      .replace(
        /(Impact factor\s*:\s*\d+(\.\d+)?)/gi,
        `<strong class="text-green-600">$1</strong>`
      )
      .replace(
        /(H-index\s*:\s*\d+)/gi,
          ...
          <p className='text-sm text-muted-foreground'>
            ...
            <span
              dangerouslySetInnerHTML={{
                __html: `Published in ${highlightKeywords(
                  conference
                )}, ${highlightKeywords(year)}`,
              }}
            />
          </p>
          {metadata && (
        ...
        <div className='mt-4 flex flex-wrap gap-4 items-center'>
          ...
          {doi && doi !== 'Not available' && (
            <p className='text-sm text-blue-600'>
              ...
              >
                <span
                  dangerouslySetInnerHTML={{ __html: highlightKeywords(doi) }}
                />
              </Link>
            </p>
          )}
